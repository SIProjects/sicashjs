...
const log = debug("sicashjs:contract")

const { logDecoder } = require("sicashjs-ethjs-abi") as IETHABI
...
export interface IContractSendRequestOptions {
  /**
   * The amount in SICASH to send. eg 0.1, default: 0
   */
  ...
  /**
   * SICash price per gas unit, default: 0.00000001, min:0.00000001
   */
...
export class Contract {
   ...
   *      [solar](https://github.com/sicashproject/solar). It includes the contract
   *      address, owner address, and ABI definition for methods and types.
   * @param opts - init options
   */
  constructor(
    private rpc: SICashRPC,
  ...
  /**
   * Executes contract method on your own local sicashd node as a "simulation"
